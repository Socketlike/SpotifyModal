export const modalElement: Element = document.createElement("div");
...
export const artistsElement: Element = document.createElement("div");
export const metadataElement: Element = document.createElement("div");
...
export function parseArtists(track: {
  artists: Array<{ name: string; id: string }>;
  name: string;
}, additionalLinkElementClasses: string): HTMLElement[] {
  const res = [document.createTextNode("by ")];
  if (track.artists.length) {
    track.artists.forEach(({ name, id }: { name: string; id: string }, index: number) => {
      ...
      if (typeof id === 'string') {
        ...
        element.classList.add(...(typeof additionalLinkElementClasses === 'string' ? additionalLinkElementClasses.split(' ') : []))
      }
